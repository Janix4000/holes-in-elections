  ...
  for (int x = 1; x < M; x++) {
    for (int y = N; y >= 0; --y) {
      ...
      for (int y_prev = N; y_prev >= y; y_prev--) {
        ...
        for (int r = 0; r < R; r++) {
          const int val =
              abs(y - votings_hist[r][x]) + dp_per_voting[x_prev][y_prev][r];
      ...
      for (int r = 0; r < R; r++) {
        dp_per_voting[x][y][r] =
            abs(y - votings_hist[r][x]) + dp_per_voting[x_prev][max_y][r];
      }
...
signed main(int argc, char** args) {
  ...
  for (size_t i = 0; i < R; i++) {
    ...
    cout << score << " ";
    cout << double(score) / (N * M) * 100 << "% ";
    // cout << "Score: " << score << endl;
    print_vec(res);
    cout << endl;
    // cout << "Score checked: " << score_across(votings_hist, res, N) << endl;
    // cout << endl << endl;
